...
filef = "Hub-Test_Failed"
files = "Hub-Test_Results"
...
def verify_erasure():
    global x
    global reading
    print(MAP, " Switching to Verify Mode...")
    while(100<=x<=104):
        reading_ser()
        if(x==100):
            print(MAP, " Checking Erasure Status...")
            verify_1()
        if(x==101):
            print(MAP, " Logging in...")
            verify_2()
        if(x==102):
            print(MAP, " Entering CLI...")
            verify_3()
        if(x==103):
            print(MAP, " Grabbing Serial...")
            verify_4()
        if(x==104):
            print(MAP, " Loading Results...")
            verify_5()
        else:
            main()
